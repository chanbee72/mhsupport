...
class linear1_model(nn.Module):
    ...
    def forward(self, inputs):
        l1 = self.get_l1(inputs)
        h = self.relu1(self.linear1(l1))
        h = self.dropout1(h)
        h = self.relu2(self.linear2(h))
        h = self.dropout2(h)
        h = self.linear3(h)
    ...
    # get linear1's output in s_model
    def get_l1(self, texts):
        l1_outputs = torch.empty((0,512)).to(device)
        for text in texts:
            sents = self.text2sentence(text)
            emb = self.to_embeddings(sents)
            emb.to(self.device)

            s_output = self.s_model.linear_n(1, emb)
            s_output = torch.mean(s_output, axis=0)
            s_output = s_output.unsqueeze(0)

            l1_outputs = torch.cat((l1_outputs, s_output))
...
model = linear1_model(s_model=s_model, in_size=512, hid_size=256, out_size=3)
model.to(device)
...
# train_dataset = Dataset(train_data['comment_text'].tolist(), (train_data['is_sscore']-1).tolist())
