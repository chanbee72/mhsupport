...
sampler = dgl.dataloading.NeighborSampler([15,10,5], prefetch_node_feats=['feat'], prefetch_labels=['es_label_v'])
...
for _ in range(200):
    ...
    for it, (input_nodes, output_nodes, blocks) in enumerate(valid_dataloader):
        with torch.no_grad():
            x = blocks[0].srcdata['feat']
            ys.append(torch.argmax(blocks[-1].dstdata['es_label_v'], dim=1))
